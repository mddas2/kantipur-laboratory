...
class reportStatus(views.APIView):
    ...
    def get(self, request):        
        if self.request.user.role == roles.SUPERADMIN or self.request.user.role == roles.SMU or self.request.user.role == roles.ADMIN:
            ...
            total_users = 0#CustomUser.objects.all().count()
            total_sample_forms_obj = SampleForm.objects.filter(owner_user_obj = self.request.user)
            total_sample_forms = total_sample_forms_obj.count()
...
class ProfileReportStatus(views.APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    def get(self, request, user_id):        
        ...
        if user_role in [roles.SUPERADMIN,roles.SMU,roles.ADMIN]:
        ...
        elif user_role == roles.SUPERVISOR:
        ...
        elif user_role == roles.ANALYST:
            total_users = 0#CustomUser.objects.all().count()
            total_sample_forms_obj = SampleFormHasParameter.objects.filter(analyst_user = user_id).all()
            total_sample_forms = total_sample_forms_obj.count()
            ...
            sample_form_obj = SampleForm.objects.filter(sample_has_parameter_analyst__analyst_user=user_id)
            not_verified = sample_form_obj.filter(status = "not_verified").count()
            completed = sample_form_obj.filter(status = "completed").count()
            

            total_report_generated = SampleFormHasParameter.objects.filter(analyst_user=user_id, sample_form__verifier__is_verified=True).count()
            data = {
                'total_request':total_sample_forms,
                'pending':pending,
                'recheck' : recheck,
        ...
        elif user_role in [roles.USER,roles.INSPECTOR]:
            total_users = 0#CustomUser.objects.all().count()
            total_sample_forms_obj = SampleForm.objects.filter(owner_user_obj = self.request.user.email)
            total_sample_forms = total_sample_forms_obj.count()
            total_report_generated = total_sample_forms_obj.filter(verifier__is_verified=True).count()
            not_verified = total_sample_forms_obj.filter(verifier__is_verified=False).count()
            recheck = total_sample_forms_obj.filter(raw_datasheet__status="rechecks").count()
