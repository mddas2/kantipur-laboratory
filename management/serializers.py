...
class SuperVisorSampleFormWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if action == "create":
            SampleForm.objects.filter(id=attrs.get('sample_form').id).update(smu = self.context['request'].user)
...
class SuperVisorSampleFormRetrieveSerializer(serializers.ModelSerializer): 
    ...
    class Meta:
        model = SuperVisorSampleForm
        fields = ['sample_form','status','parameters','created_date','is_supervisor_sent','is_analyst_test'] #sample_has_parameter_analyst
