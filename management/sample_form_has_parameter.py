...
from .serializers import SampleFormHasParameterReadSerializer,SampleFormHasParameterWriteSerializer,SampleFormReadAnalystSerializer
...
class SampleFormHasParameterViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == roles.ANALYST:
            return SampleFormHasParameter.objects.filter(status="processing",analyst_user = user,is_supervisor_sent=False).order_by("-created_date")       
        else:
